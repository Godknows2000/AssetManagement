...
public class EditModel : SysPageModel
{
    ...
    public async Task OnGetAsync()
    {
        ...
        Employee = await Db.Employees
         ...
         .Include(c => c.Title)
         .FirstAsync(c => c.Id == CurrentUserId);
    ...
    {
        Employee = Db.Employees.First(c => c.Id == CurrentUserId);
        if (await TryUpdateModelAsync(Employee, nameof(Employee), p => p.TitleId, p => p.ProfilePictureId, p => p.FirstName, p => p.Surname, p => p.DoB, p => p.IdNumber, p => p.EcNumber, p => p.Address, p => p.Position))
        {
